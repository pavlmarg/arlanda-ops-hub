...
function visitSubschemas(document, initialState, visitor) {
    ...
    var visitSubschema = function (subschema, path, states) {
        ...
        var additionalItems = subschema.additionalItems, items = subschema.items, // schema or array
        contains = subschema.contains, properties = subschema.properties, patternProperties = subschema.patternProperties, additionalProperties = subschema.additionalProperties, dependencies = subschema.dependencies, propertyNames = subschema.propertyNames, allOf = subschema.allOf, anyOf = subschema.anyOf, oneOf = subschema.oneOf, not = subschema.not, ifSchema = subschema["if"], thenSchema = subschema.then, elseSchema = subschema["else"], definitions = subschema.definitions, rest = __rest(subschema, ["additionalItems", "items", "contains", "properties", "patternProperties", "additionalProperties", "dependencies", "propertyNames", "allOf", "anyOf", "oneOf", "not", "if", "then", "else", "definitions"]);
        ...
        if (!stop && items !== undefined) {
            if (Array.isArray(items)) {
                stop = visitList(items, __spreadArray(__spreadArray([], __read(path), false), ['/items'], false), states);
            }
            else {
        ...
        if (!stop && dependencies !== undefined) {
            try {
                for (var _b = __values(Object.entries(dependencies)), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var _d = __read(_c.value, 2), key = _d[0], subschema_1 = _d[1];
                    if (typeof subschema_1 === 'object') {
                        stop = Boolean(visitSubschema(subschema_1, __spreadArray(__spreadArray([], __read(path), false), ["/dependencies/".concat((0, json_pointer_1.escapeReferenceToken)(key))], false), states));
                        if (stop) {
                            break;
                        }
                    }
