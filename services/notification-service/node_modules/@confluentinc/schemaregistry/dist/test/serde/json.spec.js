...
(0, globals_1.describe)('JsonSerializer', () => {
    ...
    (0, globals_1.it)('guid in header', async () => {
        ...
        let ser = new json_1.JsonSerializer(client, serde_1.SerdeType.VALUE, {
            autoRegisterSchemas: true,
            validate: true,
            schemaIdSerializer: serde_1.HeaderSchemaIdSerializer
        });
        let obj = {
            intField: 123,
            doubleField: 45.67,
    ...
    (0, globals_1.it)('cel field transform with nullable', async () => {
        let conf = {
            baseURLs: [baseURL],
            cacheCapacity: 1000
        };
    ...
    (0, globals_1.it)('cel field transform with union of refs', async () => {
        let conf = {
            baseURLs: [baseURL],
            cacheCapacity: 1000
        };
        ...
        let deser = new json_1.JsonDeserializer(client, serde_1.SerdeType.VALUE, deserConfig);
        let obj2 = await deser.deserialize(topic, bytes);
        (0, globals_1.expect)(obj2.messageType).toEqual(obj.messageType);
        (0, globals_1.expect)(obj2.version).toEqual(obj.version);
        (0, globals_1.expect)(obj2.payload.messageId).toEqual('12345-suffix');
        (0, globals_1.expect)(obj2.payload.timestamp).toEqual(obj.payload.timestamp);
    });
    (0, globals_1.it)('cel field transform allOf', async () => {
        let conf = {
            baseURLs: [baseURL],
            cacheCapacity: 1000
        };
        ...
        let deser = new json_1.JsonDeserializer(client, serde_1.SerdeType.VALUE, deserConfig);
        let obj2 = await deser.deserialize(topic, bytes);
        (0, globals_1.expect)(obj2.pins.pin).toEqual('P123456789-suffix');
        (0, globals_1.expect)(obj2.pins.npin).toEqual('NP00012345678-suffix');
    });
    (0, globals_1.it)('cel field transform nested anyOf', async () => {
        let conf = {
            baseURLs: [baseURL],
            cacheCapacity: 1000
        };
    ...
    (0, globals_1.it)('cel field transform sibling anyOf', async () => {
        let conf = {
            baseURLs: [baseURL],
            cacheCapacity: 1000
        };
...
(0, globals_1.describe)('JsonSerdeWithAssociatedNameStrategy', () => {
    (0, globals_1.it)('serializes and deserializes with associated name strategy', async () => {
        ...
        const info = { schemaType: 'JSON', schema: demoSchema };
        const id = await client.register('my-custom-subject', info, false);
        (0, globals_1.expect)(id).toBeGreaterThan(0);
        const request = {
            resourceName: 'topic1',
            resourceNamespace: '-',
        ...
        const ser = new json_1.JsonSerializer(client, serde_1.SerdeType.VALUE, serConfig);
        const obj = {
            intField: 123,
            doubleField: 45.67,
            stringField: 'hi',
        ...
        const deser = new json_1.JsonDeserializer(client, serde_1.SerdeType.VALUE, deserConfig);
    ...
    (0, globals_1.it)('falls back to topic name strategy when no association found', async () => {
        ...
        const bytes = await ser.serialize(topic, obj);
        const deser = new json_1.JsonDeserializer(client, serde_1.SerdeType.VALUE, {});
        const obj2 = await deser.deserialize(topic, bytes);
        (0, globals_1.expect)(obj2).toEqual(obj);
    });
...
(0, globals_1.describe)('JsonSerdeWithCustomSubjectNameStrategy', () => {
    (0, globals_1.it)('invokes a user-supplied subjectNameStrategy function', async () => {
        const conf = { baseURLs: [baseURL], cacheCapacity: 1000 };
        const client = schemaregistry_client_1.SchemaRegistryClient.newClient(conf);
        ...
        const bytes = await ser.serialize(topic, obj);
        (0, globals_1.expect)(invocations).toBeGreaterThan(0);
        const deserConfig = {
